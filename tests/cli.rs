...
fn starts_and_prints_help() {
    ...
    assert!(stdout.contains("intent import <policy-file> --format selinux|apparmor [--explain]"));
    assert!(stdout.contains("intent diff <original-policy> <regenerated-policy>"));
...
#[test]
fn import_explain_describes_selinux_mappings_and_extensions() {
    let output = intent()
        .args([
            "import",
            "tests/fixtures/import_realworld_selinux.te",
        ...
        .output()
        .expect("intent import --explain selinux should run");

    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    assert!(stdout.contains("Import explanation"));
    assert!(stdout.contains("Mapped:"));
    assert!(stdout.contains("allow himmelblaud_t himmelblau_var_cache_t:file"));
    assert!(stdout.contains("-> storage.cache"));
    assert!(stdout.contains("confidence: 97%"));
    assert!(stdout.contains("corenet_tcp_connect_http_port(himmelblaud_t)"));
    assert!(stdout.contains("-> network.outbound:https"));
...
#[test]
fn import_explain_describes_apparmor_mappings_and_extensions() {
    let output = intent()
        .args([
            "import",
            "tests/fixtures/import_realworld.apparmor",
        ...
        .output()
        .expect("intent import --explain apparmor should run");

    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    assert!(stdout.contains("Import explanation"));
    assert!(stdout.contains("/var/cache/demo-service/** rwk,"));
    assert!(stdout.contains("-> storage.cache"));
    assert!(stdout.contains("network inet stream,"));
    assert!(stdout.contains("-> network.outbound:https"));
    assert!(stdout.contains("warning: AppArmor stream network rule"));
    assert!(stdout.contains("ptrace read peer=unconfined,"));
    assert!(stdout.contains("-> stored in extensions.apparmor.rules"));
}

#[test]
fn diff_reports_round_trip_policy_fidelity() {
    let original = env::temp_dir().join(format!("intent-original-{}.apparmor", std::process::id()));
    let regenerated = env::temp_dir().join(format!(
        "intent-regenerated-{}.apparmor",
        std::process::id()
    ));
    fs::write(
        &original,
...
"#,
    ...
    .expect("original policy should be written");
    fs::write(
        &regenerated,
        r#"
...
"#,
    ...
    .expect("regenerated policy should be written");
    ...
    let output = intent()
        ...
        .arg(&regenerated)
        .output()
        .expect("intent diff should run");

    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    assert!(stdout.contains("Policy diff (apparmor)"));
    assert!(stdout.contains("Matched statements: 2"));
    assert!(stdout.contains("Original coverage: 66%"));
    assert!(stdout.contains("Only in original:"));
    assert!(stdout.contains("ptrace read peer=unconfined,"));
    assert!(stdout.contains("Only in regenerated:"));
    assert!(stdout.contains("/var/cache/demo rwk,"));
}
